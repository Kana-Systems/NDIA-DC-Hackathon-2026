...
prototype. Work on `integration/acquisition-lens-sagemaker`; preserve the original local
prototype. The expanded scope includes actual local classifier training and live
GPT-5.6 Terra inference through the existing GovCloud credentials. The React
bundle is part of the GovCloud container; trained-model promotion remains gated.
...
- Promote a trained artifact into a reviewed inference image only after the
  federal benefit gate passes. SageMaker output is never promoted automatically.
